# Chapter 6: The Hardware QoS Egress (`priority_scheduler.v`)
...
However, all of these isolated queues eventually need to be funneled back onto a single physical wire—either the 100G fiber optic Ethernet link (via the CMAC) or the PCIe bus to the host server. 
...
### "Why don't we just sort the array?"
**Confusion:** A software engineer might try to "sort" an array of queues by priority in Verilog, just like using `Array.sort()` in Python.
**Explanation:** Sorting algorithms (like QuickSort or BubbleSort) require iterative execution over time or massive numbers of comparators in space. In hardware, you do not "sort" the memory. The memory stays physically static. Instead, you build a combinational "filter" (the Priority Encoder) that instantly spotlights the winner based on the current configuration states.
...
* If two queues (Queue 1 and Queue 2) are both configured to Priority 0, and both have data, which queue will the nested `for` loop select? Is this fair?
* What happens to `stat_total_packets` if a packet drops due to hardware backpressure before it reaches this module?

### 2. Things to Modify
* **Implement Round-Robin Arbitration:** Modify the Priority Encoder logic. Instead of scanning `p` from 0 to 3, implement a register `last_serviced_queue`. The `always @(*)` block should start its `for` loop from `last_serviced_queue + 1` and wrap around. This entirely removes "Strict Priority" and guarantees perfect fairness, solving the starvation problem.
* **Add a Watchdog Timer:** Add a counter that tracks how long Queue 3 has been empty. If Queue 3 has been blocked by Queue 0 for more than 10,000 clock cycles, temporarily elevate Queue 3's priority to 0 to prevent complete starvation.
