# Chapter 3: The Core Datapath Routing (`flow_classifier.v`)
...
The Parser is structurally static—the definition of an IPv4 header does not change. The Classifier is structurally dynamic—routing rules change constantly. Keeping them in separate Verilog modules adheres to the Single Responsibility Principle, allowing the routing logic to be re-synthesized or expanded without touching the parsing logic.
...
In Chapter 4, we will learn that FIFOs use BRAM. Why doesn't the rule table use BRAM? 
...
The author chose `NUM_RULES = 16`. This is small enough that the parallel matching logic and the priority encoder can resolve inside a single clock cycle without violating 250 MHz timing constraints.
...
**Confusion:** A software engineer sees the `for (j=0; j<NUM_RULES; j++)` in the Priority Encoder and thinks the FPGA iterates through a loop like a CPU, taking 16 clock cycles.
**Explanation:** Verilog is a Hardware Description Language. The synthesizer unrolls the `for` loop and physically builds a cascade of 16 hardware multiplexers connected end-to-end. The electrical signal traverses all 16 multiplexers almost instantly, completing the entire "loop" within a single nanosecond.

### "Generate blocks are if statements"
**Confusion:** A software engineer sees `genvar i; generate for...` and confuses it with a runtime loop.
**Explanation:** `generate` blocks do not exist while the FPGA is running. They are preprocessor directives. They tell the compiler "Copy and paste this block of text 16 times, replacing the letter 'i' with 0, 1, 2, etc." By the time the FPGA is flashed, the `generate` keyword has disappeared, replaced by thousands of physical logic gates.
...
* If `cfg_dst_ip_mask` is set to `32'd0` (All Zeros), what IP addresses will this rule match?
* Why does the module use `s_axis_tready = m_axis_tready || !m_axis_tvalid;` for its backpressure logic instead of just `assign s_axis_tready = m_axis_tready;`? (Hint: Skid Buffer principles).

### 2. Things to Modify
* **Add Source IP Matching:** The current rule structure only matches Destination IP. Modify the module ports to accept `cfg_src_ip` and `cfg_src_ip_mask`. Update the rule storage arrays. Finally, update the `generate` block to include a `wire src_ip_match` utilizing the newly created mask, and add it to the final `assign rule_match[i]` logic.
