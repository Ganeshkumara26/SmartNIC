# Chapter 5: Multi-Tenant Traffic Storage (`queue_manager.v`)
...
### The `+:` Part-Select Operator in `queue_fill_levels`
```verilog
    assign queue_fill_levels[g*8 +: 8] = fill[7:0];
```
**Confusion:** Beginners struggle with how 4 separate 8-bit fill levels are sent across a single wire.
**Explanation:** `queue_fill_levels` is a 32-bit wide bus. The `generate` loop uses `g`. 
* When `g=0` (URLLC), it maps to `queue_fill_levels[0 +: 8]` (bits 0 to 7).
* When `g=1` (eMBB), it maps to `queue_fill_levels[8 +: 8]` (bits 8 to 15).
This elegantly packs 4 discrete telemetry metrics into a single contiguous cable to route to the Scheduler.
...
* If the Flow Classifier assigns a packet `Slice ID = 15`, but `NUM_QUEUES = 4`, what happens in the enqueue logic? How does the `enq_qid` assignment protect the array from out-of-bounds writes?
* Why does `deq_data` require a massive 705-bit register? What happens if `TLAST` was not stored in the memory array?

### 2. Things to Modify
* **Drop Counter:** Modify the enqueue logic. If `s_axis_tvalid` is high but the target queue is full (`enq_target_full = 1`), increment a new 32-bit `packets_dropped` telemetry register.
* **Independent Queue Depths:** Currently, all queues are exactly `QUEUE_DEPTH` (64) entries deep. Modify the math so that Queue 0 (URLLC) is 16 entries deep (to enforce low buffering), while Queue 1 (eMBB) is 256 entries deep. *(Hint: You can no longer use simple multiplication for the base address; you will need an array of base offsets).*
